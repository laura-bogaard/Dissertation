...
# its going to be a negative-binomial rather than a poisson... but isn't this obvious?

# correct date format
distance$date <- as.Date(distance$date,"%m/%d/%y")

#get juliann day onto distance data
distance_X <- pred %>%
  select(session_id, dur_hr, Julian_day_from_start)
left_join(distance, pred, by = "session_id")
# append to distance data               
distance <- left_join(distance, distance_X, by = "session_id") 

###TODO comeback to this when i sort out august bearings

# select for days with invalid bearings
data_dist <- distance %>%
  select(date, hour, minute, blockid, rl, session_id,fish_day, treatment, species, num_idv, obs, behaviour, 
...
#   select(date, hour, blockid, minute, session_id, fish_day, treatment, species, num_idv, obs, behaviour,
#          foraging, block, block_ns, platform_bearing, platform_distance) %>%
#                     filter(date > "2020-09-02")
# 
# ########### okay bring in clean distance data to validate NEED TO RUN MULtiNOM GAM SCRIPT FIRST
# 
# valid_bearings <- clean_dist2 %>%
#   filter(jul_day > 39)
# 
...
# correct for values over 360 ### this could be problematic
...
# theta1 <- (valid_bearings$platform_bearing * (pi/180))
# 
# # next convert polar to cartesian
# xy_coord <- tibble(pol2cart(valid_bearings$platform_distance, theta1, degrees = F))
# 
# 
# # plot to check
...
ggplot()+
  geom_sf(data = polygon)+
  geom_sf(data = gr2)+
  geom_sf(data = valid_bearings2[!is.na(valid_bearings2$id),], aes(color = blockid))
...
# 
# # I will now merge a df with all the sesson covar i want
# mg2 <- merge(mg1, sessioncovar, all.x = T, by = "session_id")
# 
# df2$AGE <- NULL
# df2$SEX <- NULL
# df3 <- merge(mg1, sessioncovar, by = "session_id")
# 
# df3
# distinct(df3)
# 
# mg3 <- lapply( function(x) mg1[[x]][match(valid_bearings2$session_id, mg1$session_id.x)])
# 
# 
# mg2 <- valid_bearings %>%
#   select()
...
# length(unique(valid_bearings$session_id))
# 235*57
# names(sessioncovar)
# 
# 
# #create new df with session covar only
# sessioncovar <- valid_bearings2 %>%
#   select(Date, jul_day, session_id, hour, fish, treatment, dur_hr, obs)
#   
# sessioncovar <- data.frame(sessioncovar[ ,1:8])
# 
# unique(sessioncovar)
# str(sessioncovar)
# 
# # # join this with final data
# # finaldata <- left_join(final_data, valid_bearings2, by = "id")
# finaldata <- unique(finaldata)
# 
# 
# pred$session_id <- pred$Session_ID
# sessioncovar2 <- pred%>%
# select(session_id, Julian_day_from_start, dur_hr, pred_rate_per_hour, Seal_Freq, SL_Freq)
# #new session covar
# sessioncovar <- left_join(sessioncovar1, sessioncovar2, by = c("session_id"))
#sessioncovar <- distinct(sessioncovar)

# make df
valid_bearings2 <- data.frame(valid_bearings2)
gr2 <- data.frame(gr2)

#make forage model data ** digression **
foragemod_dat <- inner_join(valid_bearings2, gr2, by = "id")
foragemod_dat <- left_join(foragemod_dat, sessioncovar, by = "session_id")
#foragemod_dat <- select(foragemod_dat, - c(geometry.y, rl, date.y, fish_day.y, foragemod_dat$RL, treatment.y, obs.y, hour.y))



#match session covar to final data with presence absence
final_data1 <- inner_join(final_data, sessioncovar, by = "session_id")
final_data1 <- distinct(final_data1)
# #match grid id to spatial covar
# final_data1 <- left_join(finaldata1, gr2, by = c("id"), suffix = c(".fd", ".gr"), 
#                           match = "first", copy=FALSE)
# 
# 
# final_data1 <- distinct(final_data1)

#match grid id to session covar
final_data2 <- inner_join(final_data1, valid_bearings2, by = c("key"), 
                         suffix = c(".x", ".y"), match = "first", copy=FALSE)

## subset so all keys match
mg3 <- subset(mg1, key.x == key.y)

# # now match back to clean_dist2
# final_data3 <- left_join(clean_dist2, final_data2, by = )


head(final_data2)

##### not sure if i need the below code
## assign each grid square a block id and split into x, y,
# #TODO: resolve which grids seperate into which block ids
#
# vb_blockids <- as.factor(valid_bearings2$blockid)
# id <- valid_bearings2$id
#
# ID <- data_frame(vb_blockids,id)
#
# ID$key <- paste(vb_blockids, id, sep = ",")
#
# gr_ids <- gr2$id
#
# ID <- separate(ID, id, into = c("x", "y"), remove = F, sep = "[^[:alnum:]]+")
#
# # FINALLY add XY to final data 2
# final_data2 <- separate(final_data2, id, into = c("x", "y"), remove = F, sep = ",")
# head(final_data2)



# # now select pa = 0 and match 
# final_dat2_pa0 <- final_data1 %>%
#   filter(pa == "0")
# 
# #match grid id to session covarcovar
# final_dat2_pa0 <- merge(final_dat2_pa0, valid_bearings2, by = c("session_id")) # suffix = c(".vb", ".fd"), copy=FALSE)
# head(final_dat2_pa0)
# 
# # fix foraging covariate for 0 = not and 1 = foraging 
# summary(final_data1)
# str(final_data1)
# final_data1$pa <- as.factor(final_data1$pa)
